...
const seedDB = async () => {
  try {
    ...
    await ReadingPlan.deleteMany({ createdBy: null }); // Only delete default public seeds
    console.log('Cleared existing default public plans.');

    await ReadingPlan.insertMany(plans);
    console.log('Seeded database with default plans successfully! 🌱');

    // Seed admin & member users
    await User.deleteMany({ email: { $in: ['admin@casacuvantului.ro', 'member@casacuvantului.ro'] } });
